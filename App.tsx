...
import { CodeIcon, TrashIcon, PlusIcon, WandIcon, GridIcon, MagnetIcon, CalendarIcon, LockIcon, UnlockIcon, SettingsIcon, CopyIcon, SaveIcon } from './components/icons';
            ...
            if (control.properties.events) {
                Object.entries(control.properties.events).forEach(([eventName, eventCodeObj]) => {
                    ...
                    if (eventCode) {
                        ...
                        const code = `    # User-defined code for ${control.properties.Name}.${eventName}\n${indentedCode}`;
                        allEventHandlers.push({ formId: form.id, controlName: control.properties.Name, eventName, funcName, code });
                    }
                });
            }
        });
          ...
          <ul>
            {forms.map((form, index) => (
                <li key={form.id} 
                    ...
                    onClick={() => { setCurrentFormIndex(index); setSelectedItemIds([form.id]); setActiveTab('properties'); }}
                >
